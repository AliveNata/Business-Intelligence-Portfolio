...
## Problem
Regional e-commerce team needed one dashboard for account managers to track sales performance and target achievement across multiple brand accounts, marketplaces, and currencies, replacing manual spreadsheet reporting.
...
- `Product Performance.sql`: product and SKU-level GMV with category mapping, includes fixes for duplicate joins that previously inflated GMV figures (see in-file changelog).
...
## Impact
Account managers could self-serve daily performance versus target without waiting on the analytics team. The cancellation-reason breakdown helped operations pinpoint whether fulfillment issues were platform-side, seller-side, or buyer-side, instead of treating every cancellation the same.
