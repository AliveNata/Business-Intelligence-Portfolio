...
const GenerateDisplay = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://localhost:5000/api/generate"); // Pastikan endpoint ini benar
     ...
     const downloadExcel = () => {
        ...
        utils.book_append_sheet(workbook, worksheet, "Hasil Klasifikasi"); // Menambahkan worksheet ke workbook
        ...
        const fileName = `Tabel Klasifikasi_${formattedDate}.xlsx`;
        writeFile(workbook, fileName);
    };

    // Fungsi untuk navigasi ke TableDisplay
    ...
    return (
        <div className="table-container">
            <h2>{t("generate_data_display")}</h2>
            <div className="button-container">
                <button onClick={handleBackToTableClick} className="back-button">
                    <FontAwesomeIcon icon={faArrowLeft} style={{ marginRight: '5px' }} /> {/* Menambahkan ikon back */}
                    {t("back_to_table")}
                </button>
            </div>
            {loading ? (
                ...
                <table className="styled-table">
                    <thead>
                        <tr>
                            ...
                            <th>Total Unsur Pokok</th>
                            <th>Total Unsur Penunjang</th>
                            <th>Hasil Akhir</th>
                            <th>KETERANGAN</th>
                        </tr>
                    </thead>
                <tbody>
                    {data.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td>{item.unsur_pokok}</td>
                            <td>{item.unsur_penunjang}</td>
            ...
            <div className="button-container">
                ...
                <button onClick={handleBackToUploadClick} className="back-button">{t("back_to_upload")}</button>
            </div>
        </div>
    );
}
